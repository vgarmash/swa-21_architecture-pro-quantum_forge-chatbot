...
1. **Сравните модели эмбеддингов** (локальные Sentence-Transformers vs облачные OpenAI Embeddings):
   - скорость создания индекса
   - качество поиска
   - стоимость владения и использования
1. **Сравните векторные базы ChromaDB и FAISS**:
...
1. **Выберите рекомендуемую конфигурацию** сервера (CPU, RAM, GPU), чтобы развернуть RAG-бота.
